...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    private float m_RayHeight = 0.25f; //De ou part mon raycast
    [SerializeField]
    ...
    [SerializeField]
    private Transform m_RaycastParent; //8 RayCast pour le CanJump
    ...
    private RaycastHit m_ChildHitInfo;
    ...
    private void Update()
    {
        GetInput();
        CalculateDirection();
        RaycastGroundCanJump();
        RaycastGroundCanClimb();
        CalculateForward();
    ...
    private void RaycastGroundCanJump()
    {
        for (int i = 0; i < m_RaycastParent.childCount; i++)
        {
            if (Physics.Raycast(m_RaycastParent.GetChild(i).transform.position, m_RaycastParent.GetChild(i).transform.position - Vector3.up, m_RayHeight, LayerMask.GetMask("Ground")))
            {
                Debug.DrawLine(m_RaycastParent.GetChild(i).transform.position, m_RaycastParent.GetChild(i).transform.position - Vector3.up * (m_RayHeight), Color.cyan);
                Debug.Log("J'Suis dedans");
                m_IsGrounded = true;
                return;
            }
        }
    ...
    private void RaycastGroundCanClimb() //Avec 9 raycast il est possible qu'on ai 2 fonctions, une pour l'angle, l'autre pour s'il peut sauter
    {
        Physics.Raycast(transform.position, Vector3.down, out m_HitInfo, m_CharacterHeight + m_HeightPadding, m_Layer);
